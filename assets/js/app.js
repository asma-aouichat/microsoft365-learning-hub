...
function update(){const total=7,done=Object.values(state).filter(Boolean).length;document.querySelectorAll('.progress span').forEach(x=>x.style.width=(done/total*100)+'%');document.querySelectorAll('[data-progress-text]').forEach(x=>x.textContent=`${done}/${total} modules terminés`)}
update();
